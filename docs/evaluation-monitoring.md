...
sample run id 或 webhook payload。
Prometheus `/metrics` 还会读取 response feedback review backlog 投影，但只输出
total、reviewed、unreviewed、unresolved、unassigned unresolved、stale unresolved
和按有限状态聚合的计数。它不会输出 feedback comment、review note、assignee、
actor、user id、run id 或 conversation id。过期未解决和未分配未解决反馈会进入
`deploy/prometheus/support-agent-alerts.yml` 的值班规则。
