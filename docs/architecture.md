...
`operations_automation_executions` is also separate from `events`. It records console, cron, or on-call bot execution of server-generated automation actions with actor, action kind, status, command method/path/query summary, body key list, body hash, command fingerprint, and result summary. The ledger keeps raw command bodies and raw command results out of durable audit storage while still proving which automation was attempted and whether it succeeded. A bounded summary endpoint and `/metrics` aggregates turn this ledger into an automation health signal without exposing action ids, actors, raw commands, or raw results.
